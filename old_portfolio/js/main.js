...
function onScrollHandler() {
    ...
    $procesLines.css("stroke-dashoffset", a / 5), a - previousWinScrollTop > 0 && $(".animation-chain").each(function (b, c) {
        var d = $(c);
        if (a > d.offset().top - winHeight / 4 * 3) {
...
}), $menuItems.find("a").on("click", function (a) {
...
$titles.textillate({
    ...
    $bannerSlides.trigger("next.owl.carousel")
...
}), $(".scroll-down-indicator").on("click", function (a) {
    $("html, body").animate({
...
var $procesLines = $("svg.expertise-line>path")
    , $win = $(window)
...
$mapCanvas.length > 0 && $mapCanvas.googleMaps({
    styled: !1
    , disableDefaultUI: !1
    , latitude: 22.30425
    , longitude: 114.1715278
    , zoom: 15
}), $mapViewSwitch.on("click", function (a) {
    a.preventDefault(), $mapCanvas.hasClass("opened") ? ($mapCanvas.removeClass("opened"), $contactMain.removeClass("closed"), $mapCanvas.data("plugin_googleMaps").removeMarker()) : ($mapCanvas.addClass("opened"), $contactMain.addClass("closed"), $mapCanvas.data("plugin_googleMaps").addMarker())
});